...
#
# Name:        CWSOCICOB 
# Purpose:     Collect socwatch data for ICOB and transfer to host.
#
...
class CWSOCICOB(CTfWorkload):
    _DEFAULT_NAME = 'SOCICOB'
    ...
    def _ParamsCreate(cls):
        return dict(super(CWSOCICOB, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWSOCICOB, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\SOCICOB\Run_ICOB.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWSOCICOB, self)._AfterSubWorkload(subWorkload, currentIteration)
