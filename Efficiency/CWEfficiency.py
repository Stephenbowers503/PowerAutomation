...
class CWEfficiency(CTfWorkload):
    _DEFAULT_NAME = 'Efficiency'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWEfficiency, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r'C:\users\Administrator\Documents\TransferToSUT\Autofiles\Dut_Side\Efficiency2.vbs')
        except:
            self._Logger.Info("Didnt execute, communicator might have unspecified IP, reset Dut with dongle attached")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWEfficiency, self)._AfterSubWorkload(subWorkload, currentIteration)
