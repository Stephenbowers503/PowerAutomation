...
class CWSOC1080p(CTfWorkload):
    _DEFAULT_NAME = 'SOC1080p'
    ...
    def _ParamsCreate(cls):
        return dict(super(CWSOC1080p, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWSOC1080p, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\SOC1080p\Run_1080p.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWSOC1080p, self)._AfterSubWorkload(subWorkload, currentIteration)
