...
class CWSOC_CS(CTfWorkload):
    ...
    def _ParamsCreate(cls):
        return dict(super(CWCS, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWCS, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\CS\Run_CS.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWCS, self)._AfterSubWorkload(subWorkload, currentIteration)
