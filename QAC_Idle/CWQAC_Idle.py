...
#
# Name:        CQAC_Idle
# Purpose:     QAC_Idle command on the target for a duration of time
#
# Author:      tcase & jfrankm (converted to Maple3 by rsher) / Sbowers edit
#
# Created:     17-Mar-2015 
# -------------------------------------------------------------------------------
...
class CWQAC_Idle(CTfWorkload):
    _DEFAULT_NAME = 'QAC_Idle'
    _DEFAULT_NUM_OF_RETRIES = 1
    _DEFAULT_TRIGGER = '0'
    _DEFAULT_TIME_OUT_SEC = 3000.0
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWQAC_Idle, self)._BeforeSubWorkload(subWorkload)
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWQAC_Idle, self)._AfterSubWorkload(subWorkload, currentIteration)
    ...
    def _RunSubWorkloadSyncOrAsync(self, subWorkload):
        ...
        # subWorkload.RunAsync()
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\QAC_Idle\Run_Idle.bat'])
        except:
