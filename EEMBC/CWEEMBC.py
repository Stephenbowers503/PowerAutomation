...
#
# Name:        CWEEMBC
# Purpose:     Run EEMBC and record power
#
# Author:      tcase & jfrankm & SBowers(converted to Maple3 by rsher)
#
# Created:     17-Mar-2015/edit 03-nov-2021
# -------------------------------------------------------------------------------
...
class CWEEMBC(CTfWorkload):
    _DEFAULT_NAME = 'EEMBC'
    _DEFAULT_NUM_OF_RETRIES = 1
    _DEFAULT_TRIGGER = '0'
    _DEFAULT_TIME_OUT_SEC = 800.0
    ...
    def _ParamsCreate(cls):
        return dict(super(CWEEMBC, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWEEMBC, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\EEMBC\Run_EEMBC.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWEEMBC, self)._AfterSubWorkload(subWorkload, currentIteration)
