...
class CWSOC800p(CTfWorkload):
    _DEFAULT_NAME = 'SOC800p'
    ...
    def _ParamsCreate(cls):
        return dict(super(CWSOC800p, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWSOC800p, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\SOC800p\Run_800p.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWSOC800p, self)._AfterSubWorkload(subWorkload, currentIteration)
