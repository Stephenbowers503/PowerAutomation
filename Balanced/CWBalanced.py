...
class CWBalanced(CTfWorkload):
    _DEFAULT_NAME = 'Balanced'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWBalanced, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r'C:\users\Administrator\Documents\TransferToSUT\Autofiles\Power_Profiles\Balanced.vbs')
        except:
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWBalanced, self)._AfterSubWorkload(subWorkload, currentIteration)
