...
class CWIcob_Setup(CTfWorkload):
    _DEFAULT_NAME = 'Icob_Setup'
    _DEFAULT_NUM_OF_RETRIES = 1
    _DEFAULT_TRIGGER = '0'
    _DEFAULT_TIME_OUT_SEC = 99999
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWIcob_Setup, self)._BeforeSubWorkload(subWorkload)
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWIcob_Setup, self)._AfterSubWorkload(subWorkload, currentIteration)
    ...
    def _RunSubWorkloadSyncOrAsync(self, subWorkload):
        ...
        try:
             Communicator.ExecuteCommandOnTarget(r'C:\tools\elevate\elevate.exe -c C:\users\Local_Admin\Documents\TransferToSUT\Autofiles\Dut_Side\Icobsetup.PS1')
        except:
            self._Logger.Info("Couldnt execute Icob Setup, Check connection to dut.")
