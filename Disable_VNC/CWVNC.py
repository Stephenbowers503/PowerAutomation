...
class CWVNC(CTfWorkload):
    _DEFAULT_NAME = 'Disable_VNC'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWVNC, self)._BeforeSubWorkload(subWorkload)
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWVNC, self)._AfterSubWorkload(subWorkload, currentIteration)
    ...
    def _RunSubWorkloadSyncOrAsync(self, subWorkload):
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r'c:\tools\elevate\elevate.exe -c -w "C:\Program Files\TightVNC\tvnserver.exe" -stop -silent')
            Communicator.ExecuteCommandOnTarget(r'c:\tools\elevate\elevate.exe -c -w powershell Set-Service -Name "tvnserver" -Status stopped -StartupType Manual')
            Communicator.ExecuteCommandOnTarget(r'c:\tools\elevate\elevate.exe -c -w powershell C:\Users\Local_Admin\Documents\TransferToSut\autofiles\dut_Side\OS_Setup\UninstallVNC.ps1')
        except:
            self._Logger.Info("Didnt Stop VNC, if it is still running, reset Dut with dongle attached and try again")
