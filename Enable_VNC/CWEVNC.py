...
class CWEVNC(CTfWorkload):
    _DEFAULT_NAME = 'Enable_VNC'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWEVNC, self)._BeforeSubWorkload(subWorkload)
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWEVNC, self)._AfterSubWorkload(subWorkload, currentIteration)
    ...
    def _RunSubWorkloadSyncOrAsync(self, subWorkload):
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r'C:\tools\elevate\elevate.exe -c C:\Users\Local_Admin\Documents\TransferToSut\autofiles\VNC.msi -quiet')
        except:
            self._Logger.Info("Didn't install VNC")
