...
class CWNOWifi(CTfWorkload):
    _DEFAULT_NAME = 'NOInternet_Wifi'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWNOWifi, self)._BeforeSubWorkload(subWorkload)
    ...
    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWNOWifi, self)._AfterSubWorkload(subWorkload, currentIteration)
    ...
    def _RunSubWorkloadSyncOrAsync(self, subWorkload):
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r'c:\tools\elevate\elevate.exe -c powershell C:\Users\Local_Admin\Documents\TransferToSut\autofiles\dut_Side\wifi_NOInternet.ps1')
        except:
            self._Logger.Info("Didn't connect to wifi")
