...
class CWMM25_Setup(CTfWorkload):
    _DEFAULT_NAME = 'MM25_Setup'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWMM25_Setup, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r' C:\tools\elevate\elevate.exe -c  C:\users\Local_Admin\Documents\TransferToSUT\Autofiles\Dut_Side\Install_MM25.ps1')
        except:
            self._Logger.Info("Did not setup MM25")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWMM25_Setup, self)._AfterSubWorkload(subWorkload, currentIteration)
