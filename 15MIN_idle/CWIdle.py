...
class CWIdle(CTfWorkload):
    _DEFAULT_NAME = '15MIN+5xIdle'
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWIdle, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\idle\Run_Idle.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWIdle, self)._AfterSubWorkload(subWorkload, currentIteration)
