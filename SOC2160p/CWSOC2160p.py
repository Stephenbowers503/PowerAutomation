...
class CWSOC2160p(CTfWorkload):
    _DEFAULT_NAME = 'SOC2160p'
    ...
    def _ParamsCreate(cls):
        return dict(super(CWSOC2160p, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWSOC2160p, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            subprocess.call([r'C:\python36\lib\site-packages\evtar\maple\workloads\SOC2160p\Run_2160p.bat'])
        except:
            self._Logger.Info("Didn't open automation files")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWSOC2160p, self)._AfterSubWorkload(subWorkload, currentIteration)
