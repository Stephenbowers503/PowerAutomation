...
class CWSecurity_Tool(CTfWorkload):
    _DEFAULT_NAME = 'Security_Tool'
    ...
    def _DefaultSubWorkloads(cls):
        ...
        subWorkload.Params.Name = 'Security_Tool'
        return subWorkload,  # One tuple still has to have a comma at the end!

    @classmethod
    def _ParamsCreate(cls):
        return dict(super(CWSecurity_Tool, cls)._ParamsCreate(),
    ...
    def _BeforeSubWorkload(self, subWorkload):
        super(CWSecurity_Tool, self)._BeforeSubWorkload(subWorkload)
        ...
        try:
            Communicator.ExecuteCommandOnTarget(r' C:\tools\elevate\elevate.exe -c  C:\users\Local_Admin\Documents\TransferToSUT\Autofiles\Dut_Side\MM25_SecurityTool_And_RunSettings.ps1')
        except:
            self._Logger.Info("Did not setup MM25")

    def _AfterSubWorkload(self, subWorkload, currentIteration):
        super(CWSecurity_Tool, self)._AfterSubWorkload(subWorkload, currentIteration)
